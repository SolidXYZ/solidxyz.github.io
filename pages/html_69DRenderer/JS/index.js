...
let Resources = {
    textures: {
        "funny": ["images/funny.png", [512, 512]],
        "skybox": ["images/skybox3.png", [2048, 1536]],
    },
    models: {
        "funny": "models/funny.obj",
    },
}
//just in case modelmappen og bildemappen har felles origin kan vi skrive det inn her
for (var [i, v] of Object.entries(Resources.textures)) {
    v[0] = customorig + v[0];
}
for (var [i, v] of Object.entries(Resources.models)) {
    Resources.models[i] = customorig + v;
}
...
//bitmap er bare en sekvens med piksler altså bare et bilde
class Bitmap {
...
//ingen egne funksjoner egentlig, brukes til å holde informasjon
class Camera {
    constructor() {
...
class Engine {
    ...
    init() {
        ...
        //når vi drar med musa må vi sørge for at documentet ikke prøver å gjøre ting.
        ...
        //hente canvas og laste inn context
        this.cvs = document.getElementById("canvas");
        this.gfx = this.cvs.getContext("2d", {willReadFrequently: true});

        //vi må hente bilder og teksturer som tar litt tid, så vi har tekst som viser status
        ...
        //resBtns = resolutionbuttons, knapper som endrer oppløsningen
        ...
        //hvis noe viktig endrer seg som f.eks skjermstørrelse må vi oppdatere verdiene
        ...
        // psp[X] = postprocess, effekter som gjør ting mer realistisk, ingen foreløpig, bare render debugs
        ...
        for (const key in Resources.textures) {
            if (Object.hasOwnProperty.call(Resources.textures, key)) {
                ...
                //bilder kan hentes annerledes
                let image = new Image();
                image.src = imageURL;
                image.crossOrigin = "Anonymous"; //sus
                image.onload = () => {
...
//face er en del av overflaten på en figur altså det du ser
class Face {
...
//vector2 samme som 3 bare med x og y
class Vector2 {
    constructor(x = 0, y = 0) {
        this.x = x;
        this.y = y;
...
//vector3 holder 3 verdier (x, y, z) som representerer et punkt. har nyttige funksjoner
class Vector3 {
...
//objekt for som inneholder en posisjon og rotasjon (i 3d). den har nyttige funksjoner også
class Matrix4 {
    constructor() {
        //jeg har ikke helt styr på matrix4 enda men jobber med saken
        /*
        M[X][Y] = X forteller hvilken del den hører til, Y forteller hvilken verdi av X, f.eks del nr 2 heter M2[X]
        M = [
            M00, M01, M02, M03 //HøyreNormalX, HøyreNormalY, HøyreNormalZ, Størrelse
            M10, M11, M12, M13 //OppNormalX, OppNormalY, OppNormalZ, Størrelse
            M20, M21, M22, M23 //ForoverNormalX, ForoverNormalY, ForoverNormalZ, Størrelse
            M30, M31, M32, M33 //PosisjonX, PosisjonY, PosisjonZ, Størrelse
        ]
        */
        //helt standard matrix, m er 0 på rotasjon fordi med 1 skaper det unødvendig prosessorbruk med en tom rotasjon, m blir satt på nytt etter rotasjonsendring
        this.m00 = 1; this.m01 = 0; this.m02 = 0; this.m03 = 0; //X: 1 fordi positiv X er høyre
        this.m10 = 0; this.m11 = 1; this.m12 = 0; this.m13 = 0; //Y: 1 fordi positiv Y er opp
        this.m20 = 0; this.m21 = 0; this.m22 = 1; this.m23 = 0; //Z: 1 fordi positiv Z er bakover faktisk, ikke forover, man blir vant til det etter litt tid
        this.m30 = 0; this.m31 = 0; this.m32 = 0; this.m33 = 1; //tom posisjon, duh, m er 1 fordi det er standard, ingen gidder å sette den gang etter gang
    }
    ...
    //ser komplisert ut men det er egentlig bare samme greia hele veien
    //man multipliserer ikke en matrix egentlig, det blir som addering med det blir kalt multiplisering fordi det blir brukt multiplisering hele veien
    mulMatrix(right) {
        let res = new Matrix4();
    ...
    //litt som oversett bare at den gir oss sluttpunktet som vektor og ikke som matrix
    mulVector(right, w) {
    ...
    //oversett, fra global posisjon til lokal posisjon
    translate(x, y, z) {
        let res = new Matrix4();
...
class Game {
    ...
    update(delta) {
        ...
        //hvis mus er trykket, roter kamera
    ...
    render() {
        ...
        */
        ...
        //rendere figurer (manuelt fordi jeg tester lololol)
        this.r.transform = new Matrix4().translate(xPos + (index++ * gap), 0, zPos).rotate(0, r, r);
        this.r.transform = this.r.transform.scale(1);
        this.r.setMaterial(Resources.textures.funny, undefined, 30);
        this.r.drawModel(Resources.models.funny);

        //skybox (himmel / bakgrunn)
...
Input.curr_keys = {};
Input.mouses = [];
Input.maxMouseButtons = 5;

Input.last_scroll = false;
Input.curr_scroll = false;

Input.mousePosition = new Vector2(0, 0);
Input.mouseLastPosition = new Vector2(0, 0);
Input.mouseScroll = new Vector2(0, 0);
Input.mouseAcceleration = new Vector2(0, 0);

Input.init = function(engine) {
    //legge til signaler
    engine.cvs.addEventListener("mousedown", (e) => {
        if (e.button > Input.maxMouseButtons) return;
        Input.mouses[e.button].curr_down = true;
    }, false);
    window.addEventListener("mouseup", (e) => {
        if (e.button > Input.maxMouseButtons) return;
        Input.mouses[e.button].curr_down = false;
    }, false);
    ...
    window.addEventListener("mousemove", (e) => {
        ...
        Input.mousePosition.y = Constants.HEIGHT - (e.clientY - rect.top);
    });
    engine.cvs.addEventListener("wheel", (e) => {
        Input.mouseScroll.x = e.deltaX / 100;
        Input.mouseScroll.y = e.deltaY / 100;
        Input.curr_scroll = true;
    });
    for (let i = 0; i < Input.maxMouseButtons; i++)
    Input.mouses.push({ last_down: false, curr_down: false });
}

Input.update = function() {
    Input.mouseAcceleration.x = Input.mousePosition.x - Input.mouseLastPosition.x;
    Input.mouseAcceleration.y = Input.mousePosition.y - Input.mouseLastPosition.y;
    Input.mouseLastPosition.x = Input.mousePosition.x;
    Input.mouseLastPosition.y = Input.mousePosition.y;

    Input.last_scroll = Input.curr_scroll;
    Input.curr_scroll = false;

    Input.mouseScroll.x = 0;
    Input.mouseScroll.y = 0;

    for (let i = 0; i < Input.mouses.length; i++)
    Input.mouses[i].last_down = Input.mouses[i].curr_down;
...
Input.isMousePressed = function(button = 0) {
    return Input.mouses[button].curr_down && !Input.mouses[button].last_down;
}
...
Input.isMouseDown = function(button = 0) {
    return Input.mouses[button].curr_down;
}

Input.isKeyPressed = function(key) {
    return Input.curr_keys[key] && !Input.last_keys[key];
}
...
class DirectionalLight {
    constructor() {
        ...
        this.dirVS = new Vector3(); //direksjon localt til kamera
        this.color = 0xffffff;
    }
}
...
//model, har nyttige verdier og funksjoner som renderer 3d figurer
class Model {
    constructor(vPositions, vTexCoords, vNormals, indices) {
        ...
        for (let i = 0; i < this.indices.length; i++) {
            ...
            let face = [];
            for (let v = 0; v < 3; v++) { //et face har 3 punkter
                const pos = this.getPosition(vFace[v][0] - 1); //posisjon
                const tex = this.getTexCoord(vFace[v][1] - 1); //tekstur
                const nor = this.getNormal(vFace[v][2] - 1); //normal
                face.push(new Vertex(pos, 0xffffff, tex, nor)); //pakk hele greia i et punkt
            }
            face = new Face(face[0], face[1], face[2]); //face
            face.calcTangentAndBiTangent(); //tangent (deformering etter generell rotasjon)

            this.faces.push(face);
        }
    }

    //finn normal fra index
    getPosition(pos) {
        return new Vector3(this.vPositions[pos][0], this.vPositions[pos][1], this.vPositions[pos][2]);
    }

    //finn tekstur del fra tindex
    getTexCoord(tex) {
...
//dette er den som genererer piksler
class Renderer extends Bitmap {
    constructor(width, height, camera) {
        ...
        //variabler for shading
        {
            this.sun = new DirectionalLight();

            this.ambient = 0.25; //globalt lys
            this.specularIntensity = 1000; //refleksjon
    ...
    //fjerne ALT mann... og bytte det ut med ingenting og så sette distansen til noe stort
    clear(clearColor) {
        for (let i = 0; i < this.pixels.length; i++) {
    ...
    drawLine(v0, v1) {
        ...
        //skjekke at begge posisjonene er langt nok unna
        ...
        //finne posisjon fra punkt til kamera
        ...
        //rendere fra punkt lengst unna til punkt nærmest
        if (p1.x < p0.x) {
            let tmp = p0;
            p0 = p1;
            p1 = tmp;
    ...
    //face -> trekant, veldig simpelt
    drawFace(f) {
        this.drawTriangle(f.v0, f.v1, f.v2);
    }

    //drawtriangle bruker posisjoner LOKALT!!!!! alstå i X og Y PIKSLER!!!!!
    drawTriangle(v0, v1, v2) {
        //backfaceculling hvis slått på og synlig
        ...
        //vertex normal (har ingen visuell effekt men påvirker en face normal)
        ...
        //nå gutta nå starter vi med teksturer
        {
        ...
        for (let i = 0; i < 3; i++) {
            ...
            //hvis kanten er gjennom znear (for nærme til å rendere)
    ...
    //bruker camera space (lokalt fra kamera til objekt)
    drawTriangleVS(vp0, vp1, vp2) {
        const z0 = vp0.pos.z;
        const z1 = vp1.pos.z;
        const z2 = vp2.pos.z;

        //camera space til viewport space
    ...
    //local space -> world space (lokalt til kamera, til global posisjon)
    modelTransform(v) {
    ...
    //world space -> camera space (global posisjon til kamera)
    cameraTransform(v) {
    ...
    //render piksel, egentlig ikke, den bare setter pikselen der den skal være
    renderPixel(p, c) {
        ...
        //pixels, hvor pikslene er
        this.pixels[p.x + (Constants.HEIGHT - 1 - p.y) * this.width] = c;
        //zbuffer, viser hvor nærme en piksel er
...
//utility, ukategoriserte funksjoner
let Util = {};

//bilde data til bitmap
...
//bitmap til image data (bitmap klasse til en javascript klasse som heter image data som canvas kan bruke til å vise bilde)
...
//lerp en vektor3 mellom 2 andre
...
//lerp en vektor2 mellom 3 andre
...
//lerp en vektor3 mellom 3 andre
...
//hex vektor til rgb (0 - 1)
Util.convertColorToVectorRange2 = function(hex) {
    const r = ((hex >> 16) & 0xff) / 127.5 - 1;
    const g = ((hex >> 8) & 0xff) / 127.5 - 1;
    const b = (hex & 0xff) / 127.5 - 1;

    return new Vector3(r, g, b);
}

//hex til rgb255 vektor (0 - 255)
...
//posisjon, rotasjon og størrelse alt til en matrix
...
for (const key in Resources.models) {
    if (Object.hasOwnProperty.call(Resources.models, key)) {
        ...
        xhr.onreadystatechange = function () {
            if (xhr.readyState == 4 && xhr.status == 200) { //skjekke at vi faktisk fikk tak i modellen
                //last inn fil linje for linke
                const lines = xhr.response.split('\n'); //\n er en string function som lager en ny linje, f.eks når du trykker enter i et word dokument

                //v = polygon (punkt)
                //vt = text polygon (punkt på UV map for teksturer)
                //vn = normal (forteller hvor smooth noe er)
                //f = vertex (face, overflate som kobles av 3 eller flere polygons)
                ...
                //skjekk hva slags data linjen inneholder
                for (const line of lines) {
                    const tokens = line.split(" "); //dele opp enkeltverdiene med mellomrom
                    //bruker switch og case for alle typene
                    switch (tokens[0]) {
                        case "v":
                            let v = [];
                            for (let i = 0; i < 3; i++) //i < 3 fordi 3d!!!
                        ...
                        case "vt":
                            let tc = [];
                            for (let i = 0; i < 2; i++) //i < 2 fordi uv koordinater er 2d
                        ...
                        case "vn":
                            let vn = [];
                            for (let i = 0; i < 3; i++) //i < 3 normaler er en 3d retning
                        ...
                        case "f":
                            let f = [];
                            for (let i = 0; i < 3; i++) { //hente 3 polygons
                                let v = [];
                                for (let j = 0; j < 3; j++) //hente 3 polygons etter hvor de er plassert
                ...
                //pcen tåler ikke mye console log i noen programmer egentlig, så ikke uncomment det her
                //console.log(indices);
                Constants.loadedResources++;
